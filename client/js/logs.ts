...
class Logs {
    ...
    static thread: number = 0;
    static fetchId = 0;
    static ready = false;

    static HandleDataChange(e) : boolean {
    ...
    static FetchUpdate(id: number, off: number = 0) {
        var params = "";
        ...
        if(more) this.thread = setTimeout("Logs.FetchUpdate("+ Logs.fetchId +", "+ (+msgs[0]+1) +");", 1000);
    }

    static BindParameters(force: boolean = false) {
        if(!Logs.ready) return;
        ...
        if(!force) {
            ...
            for(var i in data)
                data[i] = (<HTMLInputElement>document.getElementById(data[i])).value;
    ...
    static Main() {
        ...
        for(var ifdg in elements) {
            (<HTMLInputElement>document.getElementById(elements[ifdg])).value = "";
        }
