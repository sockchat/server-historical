    ...
    for(uint8_t i = 0; i < segments; i++) {
        ...
        if(segs[i] == 254) {
            if(raw.length() >= this->headerSize + 2)
                segs[i] = sc::net::ntohs(raw.substr(this->headerSize));
            else {
                delete segs;
                return (this->legal = false);
            }
            ...
            if(raw.length() >= this->headerSize + 4)
                segs[i] = sc::net::ntohl(raw.substr(this->headerSize));
            else {
                delete segs;
                return (this->legal = false);
            }
...
bool sc::Message::Init(uint16_t id, std::vector<std::string> parts) {
    ...
    for(auto i = parts.begin(); i != parts.end(); ++i) {
        if(i->length() < 254)
            ...
            this->raw += (char)254;
            this->raw += sc::net::htons(i->length());
        } else if(i->length() <= 0xFFFFFFFF) {
            this->raw += (char)255;
            this->raw += sc::net::htonl(i->length());
        } else continue;
